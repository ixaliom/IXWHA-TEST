...
function exportData(format) {
    ...
    if (format === 'json') {
        // Export JSON (format complet)
        exportContent = JSON.stringify(manhwaLibrary, null, 2);
        mimeType = 'application/json';
        extension = 'json';
    } else {
        // Export TXT (format lisible)
        exportContent = manhwaLibrary.map(manhwa => 
            `Titre: ${manhwa.title}\n` +
            `Chapitres lus: ${getReadChaptersCount(manhwa)}/${manhwa.totalChapters}\n` +
            `URL de lecture: ${manhwa.readUrl}\n\n`
        ).join('---\n');
        mimeType = 'text/plain';
        extension = 'txt';
    }
